...
export const VirtualRow = (props: Props) => {
  const { item, handleDeleteRow, handleEditRow, hasImgField, handleChangeStatus } = props;
        ...
        <td className="p-0">
          <div className="flex justify-center items-center h-16 px-6">
            ...
            {item?.status?.isPending && (
              <span onClick={() => handleChangeStatus?.(item.id)} className="inline-block py-1 px-2 text-xs leading-5 font-medium text-purple-500 bg-purple-800 rounded-full">
                Pending
              </span>
            )}
            {item?.status?.isIn && (
